...
TABLES=("${PREFIX}-users" "${PREFIX}-markets" "${PREFIX}-trades" "${PREFIX}-strategies" "${PREFIX}-subscription-tiers" "${PREFIX}-referral-usage" "${PREFIX}-system-config" "${PREFIX}-leaderboard-snapshots" "${PREFIX}-cache")
        ...
        if [[ "$TABLE" == *"-users" ]]; then
            aws dynamodb create-table --table-name "$TABLE" \
             ...
             aws dynamodb create-table --table-name "$TABLE" \
                ...
                --billing-mode PAY_PER_REQUEST --region "$REGION"
        elif [[ "$TABLE" == *"-subscription-tiers" ]]; then
             aws dynamodb create-table --table-name "$TABLE" \
             ...
             echo "Waiting for table $TABLE to become active before enabling TTL..."
...
aws lambda update-function-configuration --function-name "$LAMBDA_NAME" \
    --environment "Variables={USERS_TABLE=${PREFIX}-users,MARKETS_TABLE=${PREFIX}-markets,TRADES_TABLE=${PREFIX}-trades,STRATEGIES_TABLE=${PREFIX}-strategies,SUBSCRIPTION_TIERS_TABLE=${PREFIX}-subscription-tiers,REFERRAL_USAGE_TABLE=${PREFIX}-referral-usage,SYSTEM_TABLE=${PREFIX}-system-config,LEADERBOARD_TABLE=${PREFIX}-leaderboard-snapshots,CACHE_TABLE=${PREFIX}-cache,MOCK_AUTH=false,PAPER_TRADING=true,FRONTEND_URL=https://$CF_DOMAIN,ENVIRONMENT=$ENV}" --region "$REGION" > /dev/null
