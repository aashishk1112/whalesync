...
const Settings = () => {
    const { settings, updateCapital, addSource, toggleSource, terminateSource, linkPolymarket, wipeUserData, deactivateAccount, acceptDisclosure } = useContext(PortfolioContext);
        ...
        <div className="container mt-4 animate-fade-in" style={{ padding: '2rem 1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '2rem', borderBottom: '1px solid var(--border)', paddingBottom: '0.5rem' }}>
                <button
                    ...
                    className={activeTab === 'trading' ? 'btn-primary' : 'btn-outline'}
                    style={{
                        padding: '0.5rem 1.5rem',
                        background: activeTab === 'trading' ? 'var(--primary)' : 'transparent',
                        border: activeTab === 'trading' ? 'none' : '1px solid var(--border)'
                    }}
                >
                    <Activity size={16} style={{ display: 'inline', marginRight: '6px' }} /> Trading
                </button>
                <button
                ...
                >
                    <UserPlus size={16} style={{ display: 'inline', marginRight: '6px' }} /> Profile
                </button>
            </div>
            ...
            <div>
                <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', background: 'rgba(255,255,255,0.03)', padding: '0.5rem', borderRadius: '12px', width: 'fit-content' }}>
                    <button 
                        ...
                        className={tradingMode === 'simulation' ? 'btn-primary' : 'btn-outline'}
                    ...
                    <button 
                        ...
                        className={tradingMode === 'polymarket' ? 'btn-primary' : 'btn-outline'}
                        style={{ 
                            padding: '0.4rem 1.2rem', 
                            fontSize: '0.85rem',
                            background: tradingMode === 'polymarket' ? 'var(--danger)' : 'transparent'
                        }}
                    >
                        Polymarket Mode
                    </button>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 1fr) 2fr', gap: '2rem' }}>
                    ...
                    {/* Left Column: Context-Specific Settings */}
                        ...
                        {tradingMode === 'simulation' ? (
                            ...
                            <section className="glass-panel" style={{ padding: '1.5rem' }}>
                                <h3 style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '1.2rem', color: 'var(--danger)' }}>
                                    <Activity size={20} /> Polymarket Automation
                                </h3>

                                <p className="text-sm text-muted mb-4">
                                    Link your Polymarket account to authorize automated predictions. 
                                </p>
                                ...
                                {settings.polymarket_address ? (
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                            {settings.linked_profile?.profileImage || settings.linked_profile?.image_url ? (
                                                <img
                                                    ...
                                                    alt="Profile"
                                                    style={{ width: '48px', height: '48px', borderRadius: '50%', border: '2px solid var(--danger)' }}
                                                />
                                            ) : (
                                            ...
                                            <div style={{ flex: 1 }}>
                                                <div style={{ fontWeight: 'bold', fontSize: '1rem', color: 'var(--text-main)', wordBreak: 'break-all' }}>{settings.linked_profile?.name || 'Linked Proxy Wallet'}</div>
                                                <div className="text-[10px] text-muted font-mono truncate">{settings.polymarket_address}</div>
                                            </div>
                                        </div>
                                        <button className="btn-outline" onClick={async () => {
                                            if (window.confirm("Disconnect account? Active automations will stop.")) {
                                                await linkPolymarket(''); 
                                            }
                                        }} style={{ fontSize: '0.75rem', color: 'var(--danger)', borderColor: 'rgba(239, 68, 68, 0.3)', width: '100%' }}>
                                            Disconnect
                                        </button>
                                    </div>
                                ) : (
                                    <button 
                                        ...
                                        disabled={isLinking}
                                        style={{ width: '100%', background: 'var(--danger)', padding: '0.8rem', display: 'flex', justifyContent: 'center', gap: '0.5rem', alignItems: 'center' }}
                                    >
                                        <img src="https://polymarket.com/favicon.ico" alt="Poly" style={{ width: '18px', height: '18px', borderRadius: '50%' }} />
                                        {isLinking ? 'Linking...' : 'Connect Polymarket'}
                                    </button>
                                )}
                            </section>
                        )}
                    ...
                    {/* Right Column: Shared Copy Sources */}
                    <section className="glass-panel" style={{ padding: '1.5rem' }}>
                        ...
                        <form onSubmit={handleAddSource} style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr auto', gap: '0.75rem', marginBottom: '2rem', alignItems: 'end' }}>
                            ...
                            <button
                                className="btn-primary"
                                type="submit"
                                disabled={settings.copy_sources.length >= settings.source_slots}
                            >
                                Follow
                            </button>
                        </form>
            ...
            {activeTab === 'profile' && (
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 1fr) 2fr', gap: '2rem' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        <section className="glass-panel" style={{ padding: '1.5rem' }}>
                            <h3 style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '1.2rem' }}>
                                <SettingsIcon size={20} className="text-primary" /> Basic Information
                            </h3>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', background: 'rgba(255,255,255,0.02)', padding: '1.5rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                                {user?.picture_url ? (
                                    <img src={user.picture_url} alt={user.username} style={{ width: '64px', height: '64px', borderRadius: '12px', border: '2px solid var(--primary)' }} />
                                ) : (
                                    <div style={{ width: '64px', height: '64px', borderRadius: '12px', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--primary)' }}>
                                        {user?.username?.[0] || 'U'}
                                    </div>
                                )}
                                <div>
                                    <div style={{ fontWeight: 'bold', fontSize: '1.1rem', color: 'white' }}>{user?.username || 'Anonymous User'}</div>
                                    <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{user?.email}</div>
                                    <button 
                                        onClick={logout}
                                        style={{ marginTop: '0.75rem', background: 'none', border: 'none', color: 'var(--danger)', fontSize: '0.75rem', fontWeight: 'bold', cursor: 'pointer', padding: 0 }}
                                    >
                                        Log Out System
                                    </button>
                                </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        ...
                        <section className="glass-panel" style={{ padding: '1.5rem', border: '1px solid var(--primary-low)' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                                <SettingsIcon size={20} className="text-primary" />
                                <h3 style={{ margin: 0, fontSize: '1.2rem', color: 'var(--text-main)' }}>Account Deactivation</h3>
                            </div>
                            <div style={{ background: 'rgba(0, 212, 255, 0.05)', padding: '1rem', borderRadius: '8px', marginBottom: '1.5rem' }}>
                                <div style={{ fontWeight: 'bold', color: 'var(--primary)', marginBottom: '0.5rem', fontSize: '0.9rem' }}>Soft Data Retention</div>
                                <p className="text-xs text-muted" style={{ margin: 0 }}>
                                    Deactivating marks your account as inactive. Your history and stats are preserved, allowing you to restore your profile at a later date.
                                </p>
                            </div>
                            <button 
                                onClick={() => setShowDeactivateConfirm(true)}
                                className="btn-outline" 
                                style={{ width: '100%', color: 'var(--primary)', borderColor: 'var(--primary-low)', padding: '0.75rem' }}
                            >
                                Deactivate Account
                            </button>
                        </section>
            ...
            {showDeactivateConfirm && (
                <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/90 backdrop-blur-xl px-4 animate-in fade-in duration-300">
                    <div className="bg-slate-900 border border-primary/20 rounded-[32px] p-10 max-w-lg w-full shadow-[0_0_100px_rgba(0,212,255,0.15)] relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-10 opacity-5 group-hover:opacity-10 transition-opacity">
                            <Activity size={160} className="text-primary" />
                        </div>
                        
                        <div className="flex flex-col items-center text-center relative z-10">
                            <div className="w-20 h-20 rounded-full bg-primary/10 flex items-center justify-center text-primary mb-8 glow-pulse">
                                <Activity size={40} />
                            </div>
                            
                            <h2 className="text-2xl font-black text-white mb-4 uppercase tracking-tighter">Deactivate Profile</h2>
                            <p className="text-[11px] font-black text-primary uppercase tracking-[0.3em] mb-6">Temporary Hibernation</p>
                            
                            <p className="text-sm text-slate-400 font-medium leading-relaxed mb-10">
                                You are about to deactivate your account. Your ranking and history will be <span className="text-white font-bold">PRESERVED</span> in our encrypted archives. You can re-link this identity anytime by simply logging back in.
                            </p>
                            ...
                            <div className="flex flex-col w-full gap-4">
                                <button 
                                    ...
                                    className="w-full py-4 bg-primary text-white hover:bg-primary-hover rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all shadow-lg shadow-primary/20"
                                >
